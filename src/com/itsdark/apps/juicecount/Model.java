package com.itsdark.apps.juicecount;
...
public class Model {
	...
	public static ArrayList<JuiceData> linkDatalist;

	public static void LoadModel(SqlHandler handler) {
		linkDatalist = new ArrayList<JuiceData>();
		loadData(handler);
	}
		...
		if (c1 != null && c1.getCount() != 0) {
			if (c1.moveToFirst()) {
				do {
					...
					int count = c1.getInt(c1.getColumnIndex("count"));
					int cost = c1.getInt(c1.getColumnIndex("cost"));
					JuiceData newwonderdata = new JuiceData(name, count, cost);
	...
	private static void loadData(SqlHandler handler) {
		...
		String query = "INSERT INTO PHONE_CONTACTS(name,count,cost) values ('Statue of Liberty','img_b','The Statue of Liberty National Monument officially celebrated her 100th birthday on October 28, 1986. The people of France gave the Statue to the people of the United States over one hundred years ago in recognition of the friendship established during the American Revolution. Over years, the Statue of Liberty symbolism has grown to include freedom and democracy.')";
	...
	public static JuiceData GetbyId(String name) {

		for (JuiceData item : linkDatalist) {
